...
function render_site_title( array $args = [] ) {
	...
	return apply_filters( 'alpha/render/site/title', $html );
}
...
function render_site_description( array $args = [] ) {
	...
	return apply_filters( 'alpha/render/site/description', $html );
}
...
function render_site_link( array $args = [] ) {
	...
	return apply_filters( 'alpha/render/site/link', $html );
}
...
function render_theme_link( array $args = [] ) {
	...
	return apply_filters( 'alpha/render/theme/link', $args['before'] . $html . $args['after'] );
}
...
function render_home_link( array $args = [] ) {
	...
	return apply_filters( 'alpha/render/home/link', $args['before'] . $html . $args['after'] );
}
...
function render_cp_link( array $args = [] ) {
	...
	$html = sprintf(
		'<a class="%1$s" href="%2$s">%3$s</a>',
		esc_attr( $args['class'] ),
		esc_url( __( 'https://classicpress.net', 'alpha' ) ),
		sprintf( $args['text'], esc_html__( 'ClassicPress', 'alpha' ) )
	);
	return apply_filters( 'alpha/render/cp/link', $html );
}
